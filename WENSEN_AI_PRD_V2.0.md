...
- 当前交付物为单文件静态交互原型 `prototype-v6.5.2.html`，无真实服务端、数据库和权限网关。
- 原型用于确认页面结构、交互路径、状态流转和核心数据关系；刷新页面后新增数据不保证持久保存。
- V2.0 PRD 以 V6.5.2 已确认页面和业务规则为准，后端实现仍需补充接口、权限、数据字典和异常处理设计。
- 正式上线时，审批、合同文件、客户联系人、财务金额和审计日志必须由服务端鉴权并持久化。
...
V2.0 不设置独立产品引导页。核心业务入口、提醒和操作建议统一由工作台承载。

### 5.1.2 工作台

工作台采用三排、左右等宽布局：

1. 第一排：AI 今日工作简报、快捷入口。
2. 第二排：我的待办、业务概览。
3. 第三排：我的工时、业务提醒。

模块要求：

- AI 今日工作简报展示优先处理、风险提醒和录入建议，三类内容均可点击进入对应审批、合同或工时页面。
- 快捷入口固定依次为：创建任务、创建商机、上传已盖章合同、创建流程。
- 我的待办聚合审批、项目任务、跟进任务和催办置顶任务；默认展示 3 条，支持展开全部和收起，不设置“查看全部”。
- 我的工时展示今日已记录、本周已记录、主要项目及快速记录入口。
- 业务概览展示正式客户、进行中项目、待签/待核验合同和应收未结清金额。
- 业务提醒展示合同待签、项目风险、逾期应收等需要关注的业务事项。
...
- 客户一级页面内提供“客户”和“商机”两个可切换页面。
- 客户页支持全部客户、正式客户、准入审批中筛选；新增客户不填写地区，需填写客户负责人、联系方式和客户来源。
- 客户详情概览聚合基本信息、客户归属、主要联系人、关联合同和关联项目，不再为合同、项目设置重复 Tab。
- 关联合同和关联项目采用与基本信息一致的分行展示，并左右并排。
- 商机详情仅保留商机概览、审批记录和操作日志；跟进记录与报价管理并排放在概览底部。
- 商机顶部提供跟进任务、新版报价和创建合同入口；跟进任务默认为个人任务，并由系统在后台关联当前商机。
...
合同清单和客户详情中的“上传已盖章合同/创建合同”均在新的浏览器标签页打开独立补录页面，避免编辑期间影响其他模块使用。页面依次包含：
...
合同信息页同时展示业务来源和项目归属，不再设置独立“关联业务”Tab。合同文件与 AI 核验页允许上传草案、审批版、签署版及变更文件；签署版用于与审批通过版本进行 AI 对比。发起变更后进入可编辑状态，原合同在变更审批完成前继续有效。
...
- 创建流程统一使用居中弹窗，不使用右侧抽屉。
- 审批人点击“同意”后弹窗填写审批意见，意见选填。
- 审批人点击“拒绝”后必须在弹窗填写拒绝原因，未填写不得提交。
- 处理结果、意见/原因、操作人和时间必须写入审批日志并回写关联业务对象。
...
- 每一期记录合同、项目、应收期次、触发条件、计划日期、应收金额和币种；列表列名统一为“应收期次”。
...
- 开票记录的“开票分摊”按每条分摊明细显示“期次 + 金额”，例如“第一期 ¥100,000；第二期 ¥20,000”，不展示首期款、交付比例等付款节点文案。
...
- 如需分析单份合同利润，必须另行定义项目公共工时和成本的分摊规则；V6.5.2 暂不自动分摊。
...
以下内容不阻塞 V6.5.2 原型，但在进入研发和验收前需要定稿：
